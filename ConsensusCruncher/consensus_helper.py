...
def read_bam(bamfile, pair_dict, read_dict, csn_pair_dict, tag_dict, badRead_bam, duplex,
    ...
    """
    # Fetch data given genome coordinates  获取给定基因组坐标的数据
    if read_chr is None:
        bamLines = bamfile.fetch(until_eof=True)
    else:
        bamLines = bamfile.fetch(read_chr, read_start, read_end)     #大概意思是捕获read_chr这个染色体，从read_start到read_end中匹配上的序列
    ...
    for line in bamLines:
        # Parse out reads that don't fall within region 解析不属于该区域的读取
        ...
        mate_unmapped = [73, 89, 121, 153, 185, 137]      #FLAG值
        badRead = True
        ...
        if badRead and badRead_bam is not None:
            ...
            pair_dict[line.qname].append(line)    #qname是用来查询模板名称 the query template name
