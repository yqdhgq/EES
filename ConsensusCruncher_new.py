...
def fastq2bam(args):
   ...
   #     Move files 
    ...
    sam2.communicate()
    """
    os.system(bwa_cmd + '>' + '{}/{}.sort.sam'.format(bam_dir, filename))
    os.system(args.samtools + ' view -bhS ' + '{}/{}.1sort.bam'.format(bam_dir, filename))
    os.system(args.samtools + ' sort' + '-o' + '{}/{}.sort.bam'.format(bam_dir, filename) + '{}/{}.sort.bam'.format(bam_dir, filename))
    """

    os.system(picard + ' --INPUT ' + '{}/{}.sort.bam'.format(bam_dir, filename) +' --OUTPUT ' + '{}/{}.sorted.bam'.format(bam_dir, filename) + ' --RGID 1 ' + '--RGPL Illumina --RGLB lib1 --RGPU unit1' + ' --RGSM '+ filename )
    #格式做了修改
    # # Index BAM
...
def consensus(args):
    ...
    #这里identifier就是LargeMid_56_L005.sorted
    #print(identifier)
    ...
    """
    if args.bedfile == 'False' and args.bdelim == '|':
        sscs_cmd = "{}/ConsensusCruncher/sscs_bayes.py --infile {} --outfile {}".format(
            code_dir, args.bam, sscs)
    elif args.bedfile == 'False' and args.bdelim != '|':
        sscs_cmd = "{}/ConsensusCruncher/sscs_bayes.py --infile {} --outfile {} --bdelim {}".format(
            code_dir, args.bam, sscs, args.bdelim)
    elif args.bedfile != 'False' and args.bdelim == '|':
        sscs_cmd = "{}/ConsensusCruncher/sscs_bayes.py --infile {} --outfile {} --bedfile {}".format(
            code_dir, args.bam, sscs, args.bedfile)
    else:
        sscs_cmd = "{}/ConsensusCruncher/sscs_bayes.py --infile {} --outfile {} --bedfile {} --bdelim {}".format(
            code_dir, args.bam, sscs, args.bedfile, args.bdelim)
    
    
    print(sscs_cmd)
    os.system("python " + sscs_cmd)
    #os.system(sscs_cmd)
    ...
    if args.bedfile == 'False':
        dcs_cmd = "{}/ConsensusCruncher/dcs_bayes.py --infile {} --outfile {}".format(code_dir, sscs, dcs)
    else:
        dcs_cmd = "{}/ConsensusCruncher/dcs_bayes.py --infile {} --outfile {} --bedfile {}".format(code_dir, sscs,
                                                                                                   dcs, args.bedfile)
    print(dcs_cmd)
    os.system(dcs_cmd)
    ...
    if args.scorrect != 'False':
        ...
        if args.bedfile == 'False':
            sc_cmd = "{}/ConsensusCruncher/singleton_test.py --singleton {}".format(code_dir, sing)
        else:
            sc_cmd = "{}/ConsensusCruncher/singleton_test.py --singleton {} --bedfile {}".format(code_dir, sing,
                                                                                                       args.bedfile)
        print(sc_cmd)
        os.system(sc_cmd)
        ...
        if args.bedfile == 'False':
            dcs_sc_cmd = "{}/ConsensusCruncher/dcs_bayes.py --infile {} --outfile {}".format(code_dir, sscs_sc, dcs_sc)
        else:
            dcs_sc_cmd = "{}/ConsensusCruncher/dcs_bayes.py --infile {} --outfile {} --bedfile {}".format(
...
if __name__ == '__main__':
    ...
    #sub_b.add_argument('--cutoff', type=float, default = 0.7, help="Consensus cut-off, default: 0.7 (70%% of reads must have the "
                                                    #"same base to form a consensus).")  #test需要被注释
